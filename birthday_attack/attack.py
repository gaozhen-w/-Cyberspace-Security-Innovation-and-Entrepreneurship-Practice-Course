...
class sm3:
    ...
    def Cross_Func(self,V, M, i):
        ...
        for j in range(64):
            ...
            sh1 = self.ROL((self.ROL(A, 12) + E + self.ROL(self.Trresp(j), j % 32)) % (2 ** 32), 7)
...
if __name__ == '__main__':
    ...
    test=24 #对数据的前24位进行检测。

    hex_number=attack(test)#得到产生碰撞的前24位的16进制表示

    print('we get the collision,and the hex number is {}'.format(hex_number))
